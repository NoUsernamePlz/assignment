...
const Faq = () => {
  return (
    <div className='md:mb-[10%] relative max-md:my-[10%]'>
     ...
     <span className='text-[18px] font-medium text-[#FF5555]'>FAQ</span>
     <h3 className='text-[48px] font-bold capitalize'>Frequently asked <br/>questions</h3> 
     <div className='grid md:grid-cols-2  grid-cols-1 gap-10 my-10'>
       {
        data.map((el,i)=>(
            <div className={`p-8 ${i== 0 || i==3 || i==4?"bg-[#FF5555] text-white" :"bg-white dark:bg-black"} rounded-md flex flex-col gap-4`} key={i}>
             <h3 className='text-[28px] font-semibold'>{el.title}</h3>
              <p className='text-[18px] font-medium'>{el.desc}</p>
            </div>
        ))
       }  
     </div>
